...
def lookup_customer(customer_bank_id: str, db: Session = Depends(get_db)):
    """Resolves an opaque bank-login identifier to a customer id and email,
    for mock-bank-oauth to send an OTP to."""
    customer = (
    ...
    return {"customer_id": str(customer.id), "email": customer.email}
